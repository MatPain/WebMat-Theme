...
class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			*/
			...
			$atts           = array();
			//$atts['title']  = ! empty( strip_tags($item->title) ) ? strip_tags($item->title) : '';
			...
			// Font Awesome icons
			if ( ! empty( $icon ) ) {
				$item_output .= '<a' . $attributes . '><span class="dropdown-itemfa ' . esc_attr( $icon ) . '"></span>&nbsp;';
			} else {
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">' . _e( 'Add a menu','webmat' ) .'</a></li>';
			$fb_output .= '</ul>';
			if ( $container ) {
				$fb_output .= '</' . $container . '>';
			}
